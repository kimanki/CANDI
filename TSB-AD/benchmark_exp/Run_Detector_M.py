...
if __name__ == '__main__':
    ...
    parser.add_argument('--batch_size', type=int, default=64, help='initial value of batch size')
    parser.add_argument('--win_size', type=int, default=10, help='initial value of window size')
    ...
    Optimal_Det_HP['batch_size'] = args.batch_size
    Optimal_Det_HP['win_size'] = args.win_size
    
    print('Optimal_Det_HP: ', Optimal_Det_HP)

    hp_string = "_".join([f"{k}-{v}" for k, v in Optimal_Det_HP.items()])
    target_dir = os.path.join(args.score_dir, args.AD_Name, hp_string)
    ...
    for filename in file_list:
        ...
        if args.save:
            ...
            # Create hyperparameter string for filename
            hp_string = "_".join([f"{k}-{v}" for k, v in Optimal_Det_HP.items()])
            w_csv.to_csv(f'{args.save_dir}/{args.AD_Name}_{hp_string}.csv', index=False)