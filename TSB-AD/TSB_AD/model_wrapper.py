...
Semisupervise_AD_Pool = ['Left_STAMPi', 'SAND', 'MCD', 'Sub_MCD', 'OCSVM', 'Sub_OCSVM', 'AutoEncoder', 'CNN', 'LSTMAD', 'TranAD', 'USAD', 'OmniAnomaly', 
                        'AnomalyTransformer', 'TimesNet', 'FITS', 'Donut', 'OFA', 'MOMENT_FT', 'M2N2', 'CANDI']
...
def run_CANDI(
        data_train, data_test, win_size=10, stride=1,
        batch_size=64, epochs=100, latent_dim=64,
        lr=1e-3, ttlr=1e-3, 
        th=0.95, valid_size=0.2, infer_mode='online',
        gating_init=0.1, tta_steps=1
    ):
    from .models.CANDI import CANDI
    clf = CANDI(
        win_size=win_size, stride=stride,
        num_channels=data_test.shape[1],
        batch_size=batch_size, epochs=epochs,
